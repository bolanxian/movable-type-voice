import { b as browser, B as Buffer, y as yauzl, w as wav, s as speedometer, p as pinyinUtil, d as defineComponent, c as createVNode, a as createApp, E as EventEmitter, e as stream, V as Vue } from './vendor.172f3023.js';
...
const Main = defineComponent({
  ...
  render() {
    const vm = this;
    return createVNode('form', { class: 'main', action: 'javascript:void+0', onSubmit: vm.handleSubmit }, [
      createVNode('div', { class: 'title' }, [
        createVNode('h1', null, name)
      ]),
      createVNode('textarea', { name: 'src' }),
      createVNode('div', { style: 'margin:10px 0px;' }, [
        createVNode('input', { type: 'submit', value: '生成', name: 'gen' }),
        createVNode('span', { style: 'display:inline-block;' }, Array.from(vm.voices.keys(), key => {
          return createVNode('label', null, [
            createVNode('input', { type: 'radio', name: 'voice', value: key }), key
          ])
        }))
      ]),
      createVNode('textarea', { name: 'dest', readonly: '' }),
      createVNode('div', { style: 'margin:10px 0px;' }, [
        vm.audioSrc != null ? createVNode('audio', { src: vm.audioSrc, controls: '', style: 'display: inline-block;' }) : null,
        createVNode('span', { name: 'tip' })
      ])
    ])
  }
});
